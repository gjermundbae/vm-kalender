...
(function () {
  ...
  function teamLine(t) {
    ...
    return (
      ...
      window.VMRender.teamLabel(t) +
      "</span>"
    );
  }

  function fillCell(btn, match, selectedIds, onToggle) {
    ...
    teams.innerHTML = teamLine(match.home) + teamLine(match.away);

    const meta = document.createElement("p");
    meta.className = "bracket-match__meta";
    meta.textContent = "Kamp " + match.matchNumber + " · " + match.dateLabel;

    btn.replaceChildren(teams, meta);
    btn.addEventListener("click", () => onToggle(match.id));
  }

  function createTreeCell(cell, selectedIds, onToggle) {
    ...
    fillCell(btn, cell.match, selectedIds, onToggle);
    return btn;
  }

  function createCenterCell(match, variant, selectedIds, onToggle) {
    const btn = document.createElement("button");
    ...
    fillCell(btn, match, selectedIds, onToggle);
    btn.prepend(label);
    return btn;
  }
  ...
  function renderBracket(container, matches, selectedIds, onToggle) {
    ...
    for (const cell of cells) {
      grid.appendChild(createTreeCell(cell, selectedIds, onToggle));
    }

    const center = document.createElement("div");
    center.className = "bracket__center";
    center.appendChild(createCenterCell(final, "final", selectedIds, onToggle));
    if (bronze) {
      center.appendChild(createCenterCell(bronze, "bronze", selectedIds, onToggle));
    }
